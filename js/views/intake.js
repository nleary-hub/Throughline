...
window.Views.intake = (() => {
  ...
  function render(S, actions) {
    ...
    async function submit() {
      ...
      UI.clear(formCol); UI.clear(previewCol);
      formCol.appendChild(UI.emptyState(`Submitted — ${ref}`, "A director will triage this within a few business days. You can track its status any time from your home screen.",
        UI.button("Propose another", { onClick: () => { state.step = 1; state.title = ""; state.problem = ""; state.departments = []; drawStep1(); } })));
    }
